...
| 28 | `28_fabricantes_garantia_rma.md` | **Motor de fabricantes + garantía + RMA**: pantalla `/fabricantes` (CRUD del maestro), `fabricante_id` en el alta de producto, **garantía del fabricante por componente** en la ficha de equipo (activar→*pendiente*→confirmar) + cola `/garantias-pendientes` ("pendiente del Galarzo"), y panel de **derivaciones** (RMA externo / interno) en el expediente de incidencia. Backend `/api/fabricantes`, `/api/componentes/{id}/garantia/*`, `/api/garantias/pendientes`, `/api/incidencias/{id}/derivaciones`, `PATCH /api/derivaciones/{id}`. |
| 29 | `29_categoria_componente.md` | Eje **categoría de componente** (Instrumento/Mass Interconnect/Wiring/Accesories): selector en catálogo (solo si tipo=componente), badge en ficha de equipo, filtros. `GET /api/productos?categoria_componente=` y `/api/componentes?categoria_componente=`. |
| 30 | `30_fix_categoria_solo_equipo.md` | **Fix**: el eje `categoria` (prompt 15) es solo de equipo, no de componente. |
| 31 | `31_editar_serie_componente.md` | Acción "Editar nº de serie" en cada fila de componente de la ficha → `PATCH /api/componentes/{id}` `{numero_serie}`. |
| 32 | `32_obsolescencia.md` | **Obsolescencia (catálogo)**: badge de estado de ciclo de vida en catálogo (`EstadoCicloBadge`), pantalla `/obsolescencia` (KPIs + cambios), `url_obsolescencia` editable en fabricante. `GET /api/obsolescencia`. Solo lectura (clasifica el agente). |
| 33 | `33_report_obsolescencia_banco.md` | **Report de obsolescencia por banco**: en la ficha de equipo, dialog con el estado de ciclo de vida de todos sus componentes + exportar **Excel/PDF** + botón **Refrescar** (re-verificación lenta). `GET /api/equipos/{id}/obsolescencia`, `.../export?formato=xlsx\|pdf`, `POST .../refrescar`. Reutiliza `EstadoCicloBadge` del 32. |
...
| 41 | `41_modal_edicion_2col.md` | **Modal de edición de producto: 2 columnas + footer fijo** (solo layout en `catalogo.tsx`): `DialogContent` más ancho (`sm:max-w-2xl`) + `max-h-[90vh]`, campos en grid de 2 columnas con scroll, `DialogFooter` (Cancelar/Guardar) siempre visible. Arregla que no se viera el botón Guardar en formularios largos. |
...
> - `GarantiaFabricante` (1:1 por componente) = `{id, componente_id, fabricante_id|null, estado (no_aplica|pendiente_activacion|activada|rechazada), fecha_solicitud|null, fecha_activacion|null, meses_garantia|null, referencia_fabricante|null, responsable|null, fecha_fin|null, estado_cobertura (sin_activar|vigente|por_vencer|vencida)}`. `POST /api/componentes/{id}/garantia/activar` (201, body `{meses_garantia?, responsable?}`), `POST .../confirmar` (200, body `{fecha_activacion, referencia?}`; 404 sin iniciar, 409 si no pendiente), `GET .../garantia` (404 si no hay), `GET /api/garantias/pendientes`.
> - `Derivacion` = `{id, incidencia_id, tipo (externa_fabricante|interna_departamento), fabricante_id|null, departamento|null, tu_referencia ("RMA-NNNN"), referencia_externa|null, estado (pendiente|enviada|en_proveedor|recibida|cerrada), fecha_creacion, fecha_envio|null, fecha_cierre|null, notas|null}`. `GET/POST /api/incidencias/{id}/derivaciones` (POST 201, body `{tipo, fabricante_id?, departamento?, notas?}`; 409 externa-sin-fabricante/interna-sin-departamento), `PATCH /api/derivaciones/{id}` (body `{estado?, referencia_externa?, notas?}`; estado avanza 1 paso o se mantiene→409 si salta/retrocede; al cerrar **resuelve la incidencia**, refrescar expediente).
> - Claves de ayuda ya sembradas: `fabricantes.maestro`, `garantia.activar`, `garantia.confirmar`, `derivaciones.crear`.
