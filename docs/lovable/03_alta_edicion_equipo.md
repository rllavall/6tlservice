...
`{ numero_serie, producto_id, cliente_id?, fecha_fabricacion?, fecha_entrega?, estado?, notas? }`, `estado` por defecto "operativo".
Campos:
...
- **Cliente:** select (no texto libre) poblado con `GET /api/clientes` (envías `cliente_id`). Opcional. Si no hay clientes, enlace a `/clientes` ("Crea primero el cliente"). Error 404 "Cliente no encontrado" si el id no existe.
...
Carga el equipo (`GET /api/equipos/{id}` → usa `equipo`). Edita solo campos mutables → `PUT /api/equipos/{id}`, body `EquipoUpdate` (parcial): `{ cliente_id?, fecha_fabricacion?, fecha_entrega?, estado?, notas? }`. El cliente se edita con el mismo select de `GET /api/clientes`.
